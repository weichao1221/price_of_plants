...
async def creat_reports_zip_first(request: Request):
    ...
    try:
        ...
        username = 'Guest'
    return templates.TemplateResponse("Quick_progress_report_xd_first_time_3_4.html", {"request": request, 'username': username})
...
async def creat_reports_xd_result(request: Request,
    ...
    try:
        ...
        username = 'Guest'
    user_folder_path = user_folder(username)
    print(f"用户文件夹是:{user_folder_path}")
    start_time = time.time()
    year = datetime.datetime.now().strftime("%Y")
    month = datetime.datetime.now().strftime("%m")
    function_dir = function_path(user_folder_path=user_folder_path, function_name='progress_reports_xd')
    # 创建一个用于保存主程序过程文件的文件夹，我将它命名为progress_reports_xd，意味对接新点软件的进度报告
    project_dir = os.path.join(function_dir, Contract_Name, f'第{Reporting_Periods}期')
    print(f'project_dir为：{project_dir}')
    try:
    ...
    progress_reports_xd_dir = os.path.join(user_folder_path, "progress_reports_xd")
    print('工程信息存入目录为：', progress_reports_xd_dir)

    creatProjectInfoJsonFile(Contract_Name=Contract_Name,
    ...
    fujian4_save_path = fujian4(
        ...
        output_path=output_folder_path, input_ht_dx=ht_folder, user_folder_path=user_folder_path,
    )
...
async def creat_reports_xd_result(request: Request,
    ...
    user_folder_path = user_folder(username)
    function_dir = function_path(function_name='progress_reports_xd', user_folder_path=user_folder_path)
    project_dir = os.path.join(function_dir, Contract_Name, f'第{Reporting_Periods}期')
    output_folder_path = creat_folder(os.path.join(project_dir, "output_files"))  # 输出文件路径

    project_info_dict = readProjectInfoJsonFile(function_dir=function_dir)[Contract_Name]
    # 创建附件2
    create_fujian2(
        output_path=output_folder_path,
...
async def creat_reports_zip_not_first(request: Request):
    try:
        ...
        username = 'Guest'
    contract_name = []
    function_dir = function_path(function_name='progress_reports_xd', user_folder_path=user_folder(username))
    data = readProjectInfoJsonFile(function_dir=function_dir)
    print(data)
    for key in data.keys():
        print(key)
        contract_name.append(key)
    print(contract_name)
...
async def creat_reports_xd_result_old_data(request: Request,
    ...
    try:
        ...
        username = 'Guest'
    user_folder_path = user_folder(username)  #用户文件夹
    function_dir = function_path(user_folder_path=user_folder_path,function_name='progress_reports_xd')   #功能文件夹
    project_dir = os.path.join(function_dir, Contract_Name, f'第{Reporting_Periods}期')   # 工程文件夹

    # 调用历史数据
    project_name_dict = readProjectInfoJsonFile(function_dir=function_dir)[Contract_Name]
    ...
    print('开始处理连接上期数据')
    project_last_dir = os.path.join(os.path.dirname(__file__), "user_folder", f"{username}", "progress_reports_xd",
                                    Contract_Name,
    ...
    fujian4_save_path = fujian4(
        ...
        danwei_path_audited=audited_excel_file_path_dw,
        output_path=output_folder_path,
        input_ht_dx=ht_folder, user_folder_path=user_folder_path
    )
